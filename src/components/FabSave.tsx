...
function FabSave() {
  ...
  const { cmsPortfolioContent, controlledAuth, readyToSaveToDb } = useSelector(
    (state: rootT) => state
  );
  const reduxStore = useSelector((state: rootT) => state);
  ...
  useEffect(() => {
    if (renderCount.current === 1)
      //when page loads the state won't be toggled
  ...
  const onClickSubmit = async () => {
    if (readyToSaveToDb) {
      const updated = await updateContent(
        cmsPortfolioContent,
        reduxStore.rawCrystalData,
        reduxStore.rawCrystalDataForHome,
        controlledAuth
      );
      if (updated) dispatch({ type: READY_TO_SAVE, payload: false });
    } else dispatch({ type: READY_TO_SAVE, payload: "err" });
  };
    ...
    <div>
      <Grid item>
        <Button
          className={
            readyToSaveToDb === "err"
              ? classes.saveContentBtnErr
              : readyToSaveToDb
              ? classes.saveContentBtn
